<template>
  ...
  <DialogRoot v-if="enableZoom">
    ...
    <DialogPortal>
      ...
      />
      <DialogContent
        class="p-0 fixed left-1/2 top-1/2 z-50 grid -translate-x-1/2 -translate-y-1/2 gap-4 border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-130 data-[state=open]:zoom-in-130 sm:rounded-lg"
      >
        <DialogClose>
          <NuxtImg
            :src="refinedSrc"
