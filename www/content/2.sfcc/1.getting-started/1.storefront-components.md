...
<img
  ...
  alt="Features Image"
  class="w-full h-auto rounded-xl cursor-zoom-in transition-transform duration-200"
  onclick="document.getElementById('imgModal').showModal()"
/>

<!-- Modal -->
<dialog id="imgModal" class="backdrop:bg-black/70 p-0 rounded-xl max-w-4xl w-full border-none">
  <form method="dialog" class="flex justify-end p-4">
    <button class="text-white text-xl font-bold hover:text-red-500">&times;</button>
  </form>
  <img
    src="/salesforcestep2.png"
    alt="Zoomed Image"
    class="w-full h-auto max-h-[90vh] object-contain rounded-b-xl"
/>
</dialog>
