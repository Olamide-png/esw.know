...
# Configure ESW Pricing (SFCC)


<StepChecklist
id="sfcc-pricing-setup"
title="Setup Checklist"
:steps="[
{ title: 'Enable ESW Module', description: 'Merchant Tools → Site Preferences → Custom Preferences → ESW General Configuration', helpPrompt: 'Explain enabling the ESW module in SFCC Business Manager.' },
{ title: 'Create Shipping Methods', description: 'Merchant Tools → Ordering → Shipping Methods. Ensure name/type supported by ESW.', helpPrompt: 'What shipping method names/types does ESW support?' },
{ title: 'Map Methods per Country', description: 'Use ESW Override Shipping in ESW Checkout Configuration to map IDs to countries.' },
{ title: 'Run Pricing Feed Job', description: 'Administration → Sites → Operations → Jobs → eswRetailerPricingFeed → Run Now', helpPrompt: 'Why might the Pricing Feed job skip a day and how to verify?' },
{ title: 'Validate Storefront', description: 'Check PDP, PLP, Cart, and ESW widgets for correct currency/pricing.', blocking: true }
]"
/>
::
