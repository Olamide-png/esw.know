...
::tabs{variant="line"}
  ...
  - Price books considered for fixed pricing must be **active** and **indexed**.
  ...
  **Fallback:** If no fixed price is found, price is shown as `null` / `N/A`.

  #### Site Assignment Restriction
  Fixed Price Books must **not** be assigned to any Site; assigned books are treated as regular books and excluded from fixed rendering.

  #### Currency and Price Book Setup
  - Add non-default currencies in SFCC when using in-country fixed prices.
  - Orders store using that currency.
  - Welcome Mat currency selection disabled; default comes from **ESW Countries** custom object.
  - Enable **Is Fixed Price Model** flag on the country.

  <br>

  ::GlossaryDrawer{term="Fixed Pricing Model Examples" size="3xl" subtitle="Examples"}
  To switch to fixed pricing, enable **Is Fixed Price Model** on the target country.
  ...
  <div class="bg-teal-50 border-t-2 border-teal-500 rounded-xl shadow p-4 dark:bg-teal-800/30">
  ...
  <div class="bg-sky-50 border-t-2 border-sky-500 rounded-xl shadow p-4 dark:bg-sky-800/30">
  ...
  icon-size: 55
  ---
  Combines fixed base prices with dynamic adjustments (FX, uplift, duties, taxes) when required.
  ::
  ::
