...
```
  ...
  ::
::
<!-- keep a blank line after the final tabs closer -->
...
::tabs{variant="line"}
  ...
  ### Description
  Helper used wherever prices are calculated or displayed. Determines fixed-price vs FX-price country, applies FX, adjustments, duty, and tax as required.

  - **price**: Number or Money object.  
  - **noAdjustment**: If `true`, skip ESW adjustments.  
  - **formatted**: If `true`, returns a formatted string with currency code; otherwise a Money object.  
  - **noRounding**: If `true`, skip rounding rules.
  ...
  ### Description
  The default implementation calls the deprecated `dw.ocapi.shop.basket.calculate` hook, which logs deprecated API usage.  
  Override to use `dw.order.calculate` instead.
