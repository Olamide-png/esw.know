...
::tabs{variant="line"}
  ...
  ### Description
  Sets cookies, pricebooks, locale, and returns JSON; exposes `SetSessionCurrency` & `GetAllowedCountry`.

  ```js
  'use strict';
  // ... within SetSessionCurrency:
  ...
  function getAllowedCountry() {
    ...
    Response.renderJSON({ success: flag, country: selectedCountry, language: language, currency: currency });
  }
  ...
  ::div{label="controllers/Cart.js"}
  ### File Path
  `SiteGenesis_controllers/cartridge/controllers/Cart.js`

  ### Description
  Applies ESW shipping method, recalculates cart, adjusts threshold discounts when ESW is enabled.
  ...
  ::div{label="controllers/COCustomer.js"}
  ### File Path
  `SiteGenesis_controllers/cartridge/controllers/COCustomer.js`

  ### Description
  Login form handler; routes unregistered flow to ESW pre-order when applicable.
  ...
  function showLoginForm() {
    ...
    loginForm.handleAction({
      login: function () { app.getController('Login').LoginForm(); },
      register: function () { response.redirect(URLUtils.https('Account-StartRegister')); },
  ...
  ### Description
  Key URL selectors used across ESW flows.
  ...
  ::div{label="scripts/OnRequest.js"}
  ### File Path
  `SiteGenesis_controllers/cartridge/scripts/request/OnRequest.js`

  ### Description
  Global onRequest hook: redirect/locale sync for ESW sessions.
  ...
  ::div{label="scripts/ValidateCartForCheckout.js"}
  ### File Path
  `SiteGenesis_core/cartridge/scripts/cart/ValidateCartForCheckout.js`

  ### Description
  Sets basket status on ESW failure or restricted product scenarios.
  ...
  ### Description
  Helper used anywhere prices need to be calculated or displayed.  
  - **price**: Number or money object to convert.  
  - **noAdjustment**: If `true`, skip ESW adjustments.  
  - **formatted**: If `true`, returns string with currency code; otherwise a Money object.  
  - **noRounding**: If `true`, bypass rounding rules.  
  Applies FX where applicable and adds adjustments, duty, and tax based on ESW rules.
  ::
  
  ::div{label="scripts/calculate.js"}
  ### File Path
  `SG_core/cartridge/scripts/cart/calculate.js`

  ### Description
  Avoids applying platform promotions during ESW order calculations.
  ...
  ### Description
  The default implementation calls the deprecated `dw.ocapi.shop.basket.calculate` hook.  
  Override to use `dw.order.calculate` instead to avoid deprecated API usage logs.
  ::
::
